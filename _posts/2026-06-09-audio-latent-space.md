...
虽然 RVQ 占主导，但量化算法的实际景观比"RVQ vs lookup-free"二分要丰富得多。survey 整理出至少八类：
...
- **RVQ**——层级残差，前述主流。
- **GRVQ**（Group Residual VQ）——HiFi-Codec[^hificodec] 的设计：把 latent 在 channel 维度切分成多组，每组独立做 RVQ。这相当于把信息分布从"只走层级深度"扩成"通道宽度 × 层级深度"两个轴，缓解第一层信息过载的问题。
- **MSRVQ**（Multi-Scale RVQ）——SNAC[^snac] 的做法，**不同 RVQ 层运行在不同的时间分辨率上**。前几层高帧率（捕捉细节），后几层低帧率（捕捉粗结构）。这是变长 tokenization 在 RVQ 框架内的一种妥协实现。LLM-Codec[^llmcodec] 也用多尺度结构，但叠加了一个独特的设计：它的码本直接取自 LLM 的词向量（详见 §2.3），于是这些多尺度 token 同时是"对齐到文本词表"的——多尺度负责把信息按粗细分层，text-aligned 负责让每层 token 在 LLM 眼里像文字。
- **CSRVQ**（Cross-Scale RVQ）——ESC[^esc] 的做法，在 encoder/decoder 的多个层级都插入量化模块，逐步从粗到细 refine 重建。
- **FSQ**（Finite Scalar Quantization）[^fsq]——把每个维度独立量化到固定离散等级，不需要 nearest-neighbor lookup，也不会 collapse。在视觉里 MAGVIT-2 等用得很成功。音频上 SQ-Codec、Spectral Codecs、HARP-Net 等使用——值得注意的是 survey 的 controlled ablation 里发现 **FSQ 在 16 kHz 上 UTMOS/DNSMOS 居然超过 RVQ**，感知层面 FSQ 不见得输。
- **PQ**（Product Quantization）——把 latent 切成子向量分别量化，常用于 SSL 模型（Best-RQ[^bestrq]、USM 的 random-projection 变体）。
- **K-means** offline——拿一个预训练好的 SSL 模型（HuBERT[^hubert] / WavLM / w2v-BERT），把它的隐层特征用 k-means 聚类得到 token。这本质上不是 codec 而是"用 SSL 当 encoder + 后置离散化"，下面 2.3 会讲。
...
这一节讲两件事：**AudioLM 当初为什么要把 token 分成 semantic 和 acoustic 两套，以及后来的工作怎么把这个分工折叠进单一 codec。** AudioLM 看到的真问题（长程语义需要专门承担）是对的，它的解法——两套独立 tokenizer + 三阶段 cascade——是 2022 年条件下的权宜。我不在这里预言"semantic / acoustic 的区分会不会终结"，只顺着来龙去脉看清：这个分工是怎么来的，又是怎么被一步步收进一个 codec 里的。
...
**caption**："AudioLM 之前的 SoundStream-only AR LM 会出现的 drift。每个音都听起来对（绿色段也是流畅的语音），但说话内容从某个时刻（红色段开始）脱轨。"
...
**一个常被低估的实验观察：codebook 越多，反而越伤下游。** survey 的 controlled benchmark 反复验证了这一点——增加 codebook 数（2 → 8 → 32）的确稳定提升重建质量，但**在 ASR、IC、SE、SS 这些下游任务上几乎都拐头变差**。原因不难猜：更多 codebook 引入更多冗余维度和更高的下游建模复杂度，而下游模型容量有限。survey 的原话是 "*optimizing for reconstruction alone does not guarantee better performance on downstream tasks*"。
...
文中的判断和那些 Hot Take 都只代表我个人此刻的看法，难免有不成熟、甚至看错的地方。非常欢迎你来找我讨论、提问，或者直接指出哪里写错了。

---

## 参考文献 {#refs}

> 以下引用按正文出现顺序自动编号，点击正文上标即可跳转。arxiv 编号均经核对。
